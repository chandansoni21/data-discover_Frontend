...
//     // Auto-scroll to bottom whenever messagesByTable or activeTable changes
//     useEffect(() => {
...
//                                                 return (
//                                                     <div key={index} className="markdown-container prose prose-invert prose-sm max-w-none">
...
const ChatTable = ({ html, isDark }) => {
    ...
    return (
        <div style={{ height: data.rowData.length > 10 ? '400px' : 'auto', width: '100%', marginTop: '1rem', marginBottom: '1rem' }}>
            <AgGridReact
                theme={gridTheme}
                rowData={data.rowData}
                columnDefs={data.columnDefs}
...
const UnifiedChatInterface = ({ dbName, visibility = 'local' }) => {
    ...
    // AG Grid theme for preview modal
    const previewGridTheme = useMemo(() => {
        return isDark
            ? themeQuartz.withParams({
                backgroundColor: '#1e293b',
                foregroundColor: '#e2e8f0',
    ...
    const handleSendMessage = async () => {
        ...
        try {
            ...
            const botMsg = {
                id: Date.now() + 1,
                type: 'bot',
                text: response.data.bot_answer?.trim() || '⚠️ No response returned from server',
                sources: response.data.sources || [],
                charts: response.data.charts || null,
                timestamp: new Date()
            };
    ...
    return (
        <div className={`flex h-screen w-full overflow-hidden ${bgClass} ${textClass}`}>
            ...
            <div className="flex-1 flex flex-col relative">
                ...
                {/* Messages List */}
                <div className="flex-1 overflow-y-auto scrollbar-thin">
                    <div className="max-w-3xl mx-auto p-6 space-y-6 flex flex-col">
